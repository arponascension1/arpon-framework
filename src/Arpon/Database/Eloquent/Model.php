...
abstract class Model implements ArrayAccess, JsonSerializable
{
    ...
    protected function asDateTime($value): \DateTime
    {
        ...
        if (is_numeric($value)) {
            return \Arpon\Support\Carbon::createFromFormat('U', (string)$value);
        }
        ...
        if ($this->isStandardDateFormat($value)) {
            return \Arpon\Support\Carbon::createFromFormat('Y-m-d', $value);
        }

        // Finally, we will just assume this date is in the format used by default on
        // the database connection and use that format to create the DateTime object
        // that is returned back out to the developers after we convert it here.
        return \Arpon\Support\Carbon::createFromFormat($this->getDateFormat(), $value);
    }
    ...
    public function load($relations): self
    {
        ...
        // eagerLoadRelations returns the modified models array, but since we're passing
        // [$this], we need to get the first element back which is $this with relations loaded
        $models = $query->eagerLoadRelations([$this]);

        return $models[0] ?? $this;
    }
