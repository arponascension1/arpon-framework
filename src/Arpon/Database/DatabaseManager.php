...
use Arpon\Contracts\Database\ConnectionResolver;
...
class DatabaseManager implements ConnectionResolver
{
    ...
    protected function getConfig($key, $default = null)
    {
        ...
        // Work directly with Application container
        if ($this->app && method_exists($this->app, 'bound') && $this->app->bound('config')) {
    ...
    protected function parseConnectionName($name)
    {
        ...
        return ($name && (str_ends_with($name, '::read') || str_ends_with($name, '::write')))
    ...
    protected function makeConnection($name)
    {
        ...
        $driver = is_array($config) ? $config['driver'] : $config->get('driver');
    ...
    protected function configuration($name)
    {
        ...
        if (is_null($connections)) {
            throw new InvalidArgumentException("Database connections not configured.");
        }
    ...
    protected function configure(Connection $connection, $type)
    {
        ...
        if ($fetchMode = $this->getConfig('database.fetch')) {
    ...
    {
        return $this->getConfig('database.default', 'mysql');
    }
