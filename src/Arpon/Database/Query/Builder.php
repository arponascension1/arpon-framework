...
class Builder
{
    ...
    public function beginTransaction() { $this->dbManager->beginTransaction(); }
    public function commit() { $this->dbManager->commit(); }
    public function rollBack() { $this->dbManager->rollBack(); }

    /**
     * @throws \ReflectionException
     */
    public function paginate(int $perPage = 15, array $columns = ['*'], string $pageName = 'page', ?int $page = null): \MyFramework\Pagination\LengthAwarePaginator
    {
        ...
        return new \MyFramework\Pagination\LengthAwarePaginator(
            $results,
            $total,
            $perPage,
            $page,
